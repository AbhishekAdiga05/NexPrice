...
export function DealScoreBadgeSmall({ score, tier }) {
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[11px] font-semibold border ${TIER_STYLES[tier] || TIER_STYLES.none}`}>
      <svg className="size-2.5" viewBox="0 0 8 8" fill="currentColor" aria-hidden="true">
...
export default function DealScoreBadge({ productId, currentPrice, showLabel = true }) {
  ...
  const isNoData = !result || result.score === null;
  const cfg = isNoData ? TIER_CONFIG.none : (TIER_CONFIG[result.tier] || TIER_CONFIG.none);
  const radius = 18;
  const circumference = 2 * Math.PI * radius;
  const offset = isNoData ? circumference : circumference - (animatedScore / 100) * circumference;

  return (
    <div className="flex items-center gap-2">
      <div className="relative size-10 shrink-0">
        <svg className="size-10 -rotate-90" viewBox="0 0 44 44" role="img" aria-label={isNoData ? "Deal score: No data" : `Deal score: ${animatedScore} out of 100`}>
