...
export default function PriceChart({ productId, targetPrice, alerts }) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      let chartData = history.map((item) => ({
        date: new Date(item.checked_at || new Date()).toLocaleDateString("en-US", {
          month: "short",
          day: "numeric",
        }),
      ...
      if (chartData.length === 1) {
        ...
        const dummyDate = new Date(history[0].checked_at || new Date());
        dummyDate.setDate(dummyDate.getDate() - 1);
        chartData = [
          {
            date: dummyDate.toLocaleDateString("en-US", {
              month: "short",
              day: "numeric",
            }),
  ...
  return (
    <div className="w-full">
      ...
      <ResponsiveContainer width="100%" height={220}>
        <AreaChart data={data} margin={{ top: 5, right: 5, bottom: 5, left: -20 }} role="img" aria-label={`Price chart showing change from ${firstPrice} to ${latestPrice}`}>
          <defs>
          ...
          <Area
            ...
            strokeWidth={2}
            fill="url(#priceGrad)"
dot={{ fill: "#f97316", r: 3, stroke: "var(--color-card)", strokeWidth: 2 }}
activeDot={{ r: 6, fill: "#f97316", stroke: "var(--color-card)", strokeWidth: 2 }}
          />
        </AreaChart>
      </ResponsiveContainer>
    </div>
