...
export default function SetPriceAlert({ productId, currentPrice, currency, alerts }) {
    ...
    <form onSubmit={handleSet} className="flex items-center gap-2">
      <div className="relative flex-1 min-w-0 max-w-[160px]">
        <Input
          ...
          className="h-9 text-xs font-mono px-3 py-0 rounded-lg"
          required
        />
      </div>
      <Button
        ...
        className="h-9 px-3 text-xs shrink-0 rounded-lg"
      >
        {submitting ? (
  ...
  const compactActiveAlert = activeAlert && (
    <div className="flex items-center gap-2">
      <div className={`size-2 rounded-full ${current <= parseFloat(activeAlert.target_price) ? 'bg-emerald-500' : 'bg-indigo-400'} shrink-0`} aria-hidden="true" />
